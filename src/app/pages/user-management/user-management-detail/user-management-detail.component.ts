...
export class UserManagementDetailComponent implements OnInit {
  ...
  getDetail(id: string | null) {
    this.userService.getDetail(id)
      .subscribe(res => {
        res && (this.user = res);
        this.status = this.user.status === 1 ? "ENABLE" : "DISABLE"
        this.colorStatus = this.user.status === 1 ? "#87d068" : "red"
        this.role = this.user.role === 'ROLE_ADMIN' ? "Quản trị viên" : "Người dùng"
        this.roleColor = this.role === 'ADMIN' ? "#108ee9" : "magenta"
      })
  }
